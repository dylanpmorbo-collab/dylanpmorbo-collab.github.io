...
function digitalFootprintMarkup(item){
 ...
 return '<details class="digital-footprint" aria-labelledby="digital-footprint-title">'+
   '<summary class="digital-footprint-toggle"><span><small class="archive-code">EVIDENCIA DIGITAL // '+String(pieces.length).padStart(2,'0')+'</small><strong id="digital-footprint-title">RASTRO DIGITAL</strong></span><span class="digital-footprint-toggle-action"><span class="digital-closed-label">DESPLEGAR ↓</span><span class="digital-open-label">REDUCIR ↑</span></span></summary>'+
   ...
   '<div class="digital-filters" role="group" aria-label="Filtrar rastro digital">'+filters.map((x,i)=>'<button type="button" data-digital-button="'+x+'" aria-pressed="'+(i===0?'true':'false')+'">'+x+'</button>').join('')+'</div>'+
...
function archivePoliceReport(item){
 ...
 const physical=reports.length?'<div class="archive-report-subheading">INFORMES FÍSICOS // '+String(reports.length).padStart(2,'0')+'</div>'+
   ...
   '</div></dialog>'+
   '<dialog class="archive-police-image-dialog" aria-label="Imagen adjunta ampliada"><button type="button" data-police-image-close aria-label="Cerrar imagen ampliada">✕</button><img data-police-image-large alt=""></dialog>'+
   '<script>(function(){const block=document.currentScript.closest(".archive-police-report-block");if(!block)return;const viewer=block.querySelector("[data-police-pages]");const lightbox=block.querySelector(".archive-police-image-dialog");const large=lightbox.querySelector("[data-police-image-large]");const close=lightbox.querySelector("[data-police-image-close]");let trigger=null;viewer.addEventListener("click",event=>{const button=event.target.closest("[data-police-image-open]");if(!button)return;trigger=button;const thumb=button.querySelector("img");large.src=thumb.currentSrc||thumb.src;large.alt=thumb.alt;lightbox.showModal();close.focus();});close.addEventListener("click",()=>lightbox.close());lightbox.addEventListener("click",event=>{if(event.target===lightbox)lightbox.close();});lightbox.addEventListener("close",()=>{large.removeAttribute("src");if(trigger?.isConnected)trigger.focus();});})();<\/script>'+
